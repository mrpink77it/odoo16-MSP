...
# Ottimizzato con Auto-Tuning per PostgreSQL e Odoo
# ==============================================================================

# Disabilita completamente i prompt interattivi di apt
...
echo -e "${BLUE}======================================================================${NC}"
echo -e "${GREEN}    Odoo & OCA Auto-Installer + Hardware Tuning (Ubuntu/Debian)${NC}"
echo -e "${BLUE}======================================================================${NC}"
...
# ==============================================================================
# FUNZIONE DI AUTO-TUNING (Da odoo18-EMS.txt)
# ==============================================================================
calculate_tuning() {
    echo -e "\n${BLUE}=== Analisi risorse hardware e Auto-Tuning PostgreSQL/Odoo ===${NC}"
    TOTAL_RAM_MB=$(free -m | awk '/Mem:/ {print $2}')
    TOTAL_CPUs=$(nproc)
    
    echo "Risorse rilevate: CPU Cores = $TOTAL_CPUs, RAM Totale = ${TOTAL_RAM_MB}MB"
    ...
    echo -e "${GREEN}Parametri calcolati:${NC}"
...
# ==============================================================================
# 2. CONFIGURAZIONE UTENTI E TUNING POSTGRESQL
# ==============================================================================
echo -e "\n${BLUE}>>> Configurazione utente di sistema e PostgreSQL...${NC}"
if ! id -u $OE_USER > /dev/null 2>&1; then
    useradd -m -U -r -d $OE_HOME -s /bin/bash $OE_USER
fi

# Tuning PostgreSQL basato sull'hardware rilevato
PG_VERSION=$(pg_lsclusters -h | awk '{print $1}')
if [ -n "$PG_VERSION" ]; then
    PG_CONF_FILE="/etc/postgresql/$PG_VERSION/main/postgresql.conf"
    if [ -f "$PG_CONF_FILE" ]; then
        echo "Applicazione tuning automatico su $PG_CONF_FILE..."
        # Sostituzione sicura usando regex per evitare conflitti se i parametri sono già decommentati
...
echo -e "Odoo è accessibile su: http://$(hostname -I | awk '{print $1}'):8069"
